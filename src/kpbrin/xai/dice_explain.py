...
def generate_dice_report(
    ...
    if target_job_ids is None:
        # Modifikasi: Kita ambil dari Top-10 secara eksplisit sesuai permintaan
        target_job_ids = ranked.head(10)["job_id"].tolist()
    ...
    for rank, job_id in enumerate(target_job_ids, start=1):
        ...
        # Jika job sudah berada di atas threshold, beri target artifisial (misalnya mencapai skor Peringkat 1 + 1.0)
        job_threshold = threshold_score
        if current_score >= threshold_score:
            top1_score = float(ranked.iloc[0]["final_score"])
            job_threshold = top1_score + 1.0

        interventions = generate_interventions(
            job_id=job_id,
            job_title=job_title,
            current_score=current_score,
            threshold_score=job_threshold,
