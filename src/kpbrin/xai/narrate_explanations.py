...
SYSTEM_PROMPT = (
    ...
    "yang diberikan.\n"
    "2. JANGAN mengubah urutan kepentingan.\n"
    "3. WAJIB menggunakan format persentase kecocokan saat menyebutkan kontributor "
    "(contoh: 'Sertifikat A sangat relevan dan menyumbang 45.2% kecocokan pada job ini' atau 'Sertifikat B cocok 30.0% pada job ini'). "
    "Angka persentasenya sudah dihitung dan disediakan di bagian 'Kontributor utama' di bawah.\n"
    "4. Tulis 2-4 kalimat, nada positif tapi jujur, fokus ke 2-3 kontributor terbesar.\n"
    "5. Jangan sebut istilah teknis (SHAP, skor, embedding).\n"
    "6. Output HANYA narasinya, tanpa heading, tanpa embel-embel lain."
)
...
def build_prompt(row, shap_rows):
    ...
    for _, r in top_features.iterrows():
        feat = r['feature']
        sv = r['shap_value']
        pct = (sv / final_score) * 100 if final_score > 0 else 0
        feature_lines_arr.append(f"- {feat}: menyumbang {pct:.1f}% kecocokan")
        
    feature_lines = "\n".join(feature_lines_arr) or "- (tidak ada rincian kontributor)"
