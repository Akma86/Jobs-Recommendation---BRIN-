...
from full_pipeline_subclo import (
    ...
    load_khs, match_courses, rank_jobs_for_queries, aggregate_subclo_to_course,
)
...
# ---------------------------------------------------------------------------
# Final aggregation: KHS signal + CV... err, CERT signal -> student level
# ---------------------------------------------------------------------------
    ...
    for _, m in included.iterrows():
        ...
        for _, cj in course_jobs.iterrows():
            ...
            job_explanations[job_id].append(
                ...
                f"-> skor sub-CLO={cj['course_job_score_max']:.2f}"
            )
